...
EZ_ASSETS=/usr/local/ezlite/assets
# SOURCE 
source $EZ_ASSETS/banner.ezs
source $EZ_ASSETS/action.ezs
...
start_deamon(){
	input_code_name	
	source $EZ_SOURCE/cryptos/${CODE_NAME}/spec.ezs	
	$MN_DEAMON --data=$MN_DIR/${CRYPTO_NAME}/ --conf=$MN_DIR/${CRYPTO_NAME}/${CRYPTO_NAME}.conf && sleep 3
}
# START MASTERNODE 
start_mn(){
	input_code_name
	input_walletpassphrase
	source $EZ_SOURCE/cryptos/${CODE_NAME}/spec.ezs	
	$MN_CLI --data=$MN_DIR/${CRYPTO_NAME}/ --conf=$MN_DIR/${CRYPTO_NAME}/${CRYPTO_NAME}.conf ${START_MN} $walletpassphrase
	sleep 3;
}
# REPORT MASTERNODE
daily_rp(){
...
#!/bin/bash
source SOURCE_MN
BALANCE=$($MN_CLI --data=$MN_DIR/${CRYPTO_NAME}/ --conf=$MN_DIR/${CRYPTO_NAME}/${CRYPTO_NAME}.conf ${BALANCE_MN})
DATE=$(date +%d-%m-%y)
...
status_mn(){
	input_code_name
	source $EZ_SOURCE/cryptos/${CODE_NAME}/spec.ezs	
	$MN_CLI --data=$MN_DIR/${CRYPTO_NAME}/ --conf=$MN_DIR/${CRYPTO_NAME}/${CRYPTO_NAME}.conf ${STATUS_MN} 
}
# Getbalance
balance_mn(){
	input_code_name
	source $EZ_SOURCE/cryptos/${CODE_NAME}/spec.ezs		
	$MN_CLI --data=$MN_DIR/${CRYPTO_NAME}/ --conf=$MN_DIR/${CRYPTO_NAME}/${CRYPTO_NAME}.conf ${BALANCE_MN}
}
# Pause 
pause(){
